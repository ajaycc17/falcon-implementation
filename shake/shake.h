...
// keccak-f variables
#define c 512  // capacity
#define r 1088 // rate
#define b 1600 // block size

// the modulo function to handle both positive and negative numbers
static int modulo(int x, int y)
{
    int res = x < 0 ? (y - ((x * -1) % y)) : (x % y);
    return res;
}

// swap a byte
// for e.g. 0000 0001 to 1000 0000
// for e.g. 0100 0000 to 0000 0010
static uint8_t byte_swap(uint8_t byte)
{
...
// swap every byte of a vector(of uint8_t type) of size vec_size
static uint8_t b2lendian(uint8_t *vector, int vec_size)
{
    for (int i = 0; i < vec_size; i++)
    {
        vector[i] = byte_swap(vector[i]);
    }
}
...
static void xci_iota(uint64_t *state, int ir)
{
    ...
    for (i = 0; i < 5; i++)
    {
        for (j = 0; j < 5; j++)
        {
            state[i + 5 * j] = (s_copy[i + 5 * j]) ^ ((s_copy[(modulo((i + 1), 5)) + 5 * j] ^ 0xFFFFFFFFFFFFFFFFULL) & (s_copy[(modulo((i + 2), 5)) + 5 * j])); // this includes xci and iota
        }
    }
    state[0] ^= RC[ir];
}
static void RND(uint64_t *state, int ir)
{
    ...
    rho(state);
    pi(state);
    xci_iota(state, ir); // call every function
}
...
static void sponge_keccakf(uint8_t *in_string, uint8_t s_uint8[], uint64_t s_uint64[], uint64_t s[], int bloques)
{
    ...
    for (int j = 0; j < (bloques + 1); j++)
    {
        ...
        {
            if (i < 136)
            { // from 0 - 1087 bits (byte # 135), every i is 8 bits filled w/ string info
...
static void keyhex(uint64_t s[], int d, int slen)
{
    ...
    for (int j = 0; j < d / 8; j++)
    {
        printf("%02x", byte_swap(s[i] >> count)); // apply swap to every byte
        count -= 8;                               // counter to switch to next block
        if (count < 0)
        {
            count = 56;
            i++; // restart counter for next block
            if (i > slen)
            {
                break;
            }
